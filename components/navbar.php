<div id="preloader">
    <div class="loader">
        <div class="loader-rect"></div>
        <div class="loader-rect"></div>
        <div class="loader-rect"></div>
        <div class="loader-rect"></div>
    </div>
</div>

<header class="header-area">
    <div class="sticky-area">
        <div class="navigation">
            <div class="container">
                <div class="row">
                    <div class="col-lg-2">
                        <div class="logo" style="margin-left: -60px;">
                            <a class="navbar-brand" href="index.php"><img src="assets/img/rara blaCK.png" alt></a>
                        </div>
                    </div>
                    <div class="col-lg-10">
                        <div class="main-menu">
                            <nav class="navbar navbar-expand-lg">
                                <button class="navbar-toggler" type="button" data-toggle="collapse"
                                    data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                                    aria-expanded="false" aria-label="Toggle navigation">
                                    <span class="navbar-toggler-icon"></span>
                                    <span class="navbar-toggler-icon"></span>
                                    <span class="navbar-toggler-icon"></span>
                                </button>
                                <div class="collapse navbar-collapse justify-content-center"
                                    id="navbarSupportedContent">
                                    <ul class="navbar-nav ml-auto">
                                        <li class="nav-item">
                                            <a class="nav-link" href="index.php">Home</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="services.php">Services</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="#">Projects
                                                <i class="far fa-angle-down"></i>
                                            </a>
                                            <ul class="sub-menu">
                                                <li><a href="/hydrogen.php"></i>Hydrogen
                                                        Project</a></li>
                                                <li><a href="/field.php"></i>Field Development
                                                        Planning</a></li>
                                                <li><a href="/simulation.php"></i>Simulation
                                                        Studies</a></li>
                                            </ul>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="about.php">About Us
                                                <i class="far fa-angle-down"></i>
                                            </a>
                                            <ul class="sub-menu">
                                                <li><a href="team.php"></i>Our Team</a></li>
                                                <li><a href="partners.php"></i>Global Reach</a>
                                                </li>
                                            </ul>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="training.php">Training
                                                <i class="far fa-angle-down"></i>
                                            </a>
                                            <ul class="sub-menu">
                                                <li><a href="training.php#101"></i>CCUS</a></li>
                                                <li><a href="training.php#hydrogen"></i>Hydrogen</a></li>
                                            </ul>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="#">Highlights 
                                                <i class="far fa-angle-down"></i>
                                            </a>
                                            <ul class="sub-menu">
                                                <li>
                                                    <a href="raraenergydinner.php">
                                                        </i>
                                                        Rara Energy Networking Event
                                                    </a>
                                                </li>
                                                <li>
                                                    <a href="ciphd_2024_at_iit_dhanbad.php">
                                                        </i>
                                                        ICPHD 2024 AT IIT Dhanbad
                                                    </a>
                                                </li>
                                                <li>
                                                    <a href="podcast.php">
                                                        </i>
                                                        Energy Podcast
                                                    </a>
                                                </li>
                                               
                                            </ul>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" href="knowledge.php">Knowledge Portal</a>
                                        </li>
                                        <li class="nav-item text-center mt-3">
                                            <button class="btn"
                                                style="background-color: #ff6600; color: white; box-shadow: 3px 3px 10px rgba(0, 0, 0, 0.3); border: none; border-radius: 20px; padding: 12px 24px; font-size: 18px; font-weight: bold;">
                                                <a class="text-white text-decoration-none" href="Books.php">Book</a>
                                            </button>
                                        </li>


                                        <li class="nav-item">
                                            <!-- <a class="nav-link" href="contact.php">Contact Us</a>s -->
                                        </li>
                                        <!-- <li class="nav-item">
                                                    <a class="nav-link" href="#">Blog
                                                        <i class="far fa-angle-down"></i>
                                                    </a>
                                                    <ul class="sub-menu">
                                                        <li><a href="blog-classic.php"></i> Blog Classic</a></li>
                                                        <li><a href="blog-grid.php"></i> Blog Grid</a></li>
                                                        <li><a href="single-blog.php"></i> Blog Details</a></li>
                                                    </ul>
                                                </li> -->
                                        <!-- <li class="nav-item">
                                                    <a class="nav-link active" href="#">Home
                                                        <i class="far fa-angle-down"></i>
                                                    </a>
                                                    <ul class="sub-menu">
                                                        <li><a href="index.php"></i> Home Demo 01</a></li>
                                                        <li><a href="index2.php"></i> Home Demo 02</a></li>
                                                        <li><a href="index3.php"></i> Home Demo 03</a></li>
                                                    </ul>
                                                </li> -->
                                        <!-- <li class="nav-item">
                                                    <a class="nav-link" href="about.php">About</a>
                                                </li> -->
                                        <!-- <li class="nav-item">
                                                    <a class="nav-link" href="services-1.php">Services</a>
                                                </li>
                                                <li class="nav-item">
                                                    <a class="nav-link" href="services-1.php">Services</a>
                                                </li> -->
                                        <!-- <li class="nav-item">
                                                    <a class="nav-link" href="">Services
                                                        <i class="far fa-angle-down"></i>
                                                    </a>
                                                    <ul class="sub-menu">
                                                        <li><a href="services-1.php"></i> Services 01</a></li>
                                                        <li><a href="services-2.php"></i> Services 02</a></li>
                                                        <li><a href="single-service.php"></i> Service Details</a></li>
                                                    </ul>
                                                </li> -->

                                        <!-- <li class="nav-item">
                                                    <a class="nav-link" href="#">Pages
                                                        <i class="far fa-angle-down"></i>
                                                    </a>
                                                    <ul class="sub-menu">
                                                        <li><a href="404.php"></i> 404 Error</a></li>
                                                        <li><a href="choose-us.php"></i> Why Choose Us</a></li>
                                                        <li><a href="login.php"></i> Login</a></li>
                                                        <li><a href="register.php"></i> Register</a></li>
                                                        <li><a href="forgot-password.php"></i> Forgot Password</a></li>
                                                        <li><a href="team.php"></i> Our Team</a></li>
                                                        <li><a href="price.php"></i> Pricing</a></li>
                                                        <li><a href="faq.php"></i> FAQ</a></li>
                                                        <li><a href="privacy-policy.php"></i> Privacy Policy</a></li>
                                                        <li><a href="term-condition.php"></i> Terms & Conditions</a></li>
                                                    </ul>
                                                </li>
                                                -->
                                    </ul>
                                </div>
                            </nav>
                        </div>
                    </div>
                    <!-- <div class="col-lg-1">
                                <div class="header-right-content">
                                    <div class="search-box">
                                        <button class="search-btn"><i class="far fa-search"></i></button>
                                    </div>
                                </div>
                            </div> -->
                </div>
            </div>
        </div>
    </div>

    <div class="search-popup">
        <span class="search-back-drop"></span>
        <div class="search-inner">
            <div class="auto-container">
                <div class="upper-text">
                    <button class="close-search"><span class="far fa-times"></span></button>
                </div>
                <form method="post" action="#">
                    <div class="form-group">
                        <input type="search" name="search-field" placeholder="Search Here..." required>
                        <button type="submit"><i class="far fa-search"></i></button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</header>